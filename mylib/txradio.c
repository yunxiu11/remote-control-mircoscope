 ...
 * @details EXTERNAL FUNCTIONS
            txradio_init() - Initialise TX radio task
            nrf24l01plus_init() - Initialise NRF24L01+ radio module
            ...
            txradio_send_packet() - Send packet to TX radio
 ******************************************************************************/
#include "FreeRTOS.h"
#include "task.h"
#include "queue.h"
...
extern QueueHandle_t QueueTrimpot;
...
void join_task(void)
{
    ...
    while (1)
    {
        ...
        if (xSemaphoreTake(onboardBtnSemaphore, portMAX_DELAY) == pdPASS)
        {
            ...
            hamming_packet_encode(raw_pkt, encoded_pkt);
            nrf24l01plus_send(encoded_pkt);

            d2_on = !d2_on;
            d2_on ? REG_MFS_LED_D2_ON() : REG_MFS_LED_D2_OFF();

            d1_on = !d1_on;
            d1_on ? REG_MFS_LED_D1_ON() : REG_MFS_LED_D1_OFF();
        }
    }
}
...
void cmdselect_task(void)
{
    while (1)
    {
        ...
        if (xSemaphoreTake(SemaphoreS1, portMAX_DELAY) == pdPASS)
        {
            ...
            if (current_opcode == CMD_OPCODE_NEW ||
                ...
                current_opcode == CMD_OPCODE_SYS)
            {
                rcmext_handle_command(current_opcode);
            }
...
void cmdin_task(void)
{
    ...
    while (1)
    {
        ...
        if (xSemaphoreTake(SemaphoreS3, portMAX_DELAY) == pdPASS)
        {
            ...
            case CMD_ORG:
            {
                ...
                txradio_send_packet(CMD_ORG, CMD_OPCODE_ORG, 0);
                continue;
            }
            }
            ...
            segdisplay_write_number(current_switch_val);
...
void ledbar_task(void)
{
    while (1)
    {
        reg_lta1000g_write(current_opcode);
...
{
    SwitchbankMsg sw_msg;
    while (1)
    {
        if (xQueueReceive(QueueSwitchbank, &sw_msg, portMAX_DELAY) == pdPASS)
        {
...
void segdisplay_task(void)
{
    while (1)
    {
        ...
        segdisplay_write_number(confirmed_display_val);
...
void rgb_task(void)
{
    ...
    while (1)
    {
        ...
        {
            reg_rgb_colour_set(new_rgb_mask);
...
void trimpot_task(void)
{
    ...
    while (1)
    {
        if (xQueueReceive(QueueTrimpot, &adc_val, 0) == pdPASS)
        {
...
void txradio_send_packet(command_opcode_t cmd_type, uint8_t opcode, int val)
{
    nrf24l01plus_init();
    uint8_t raw_pkt[16] = {0};
    uint8_t encoded_pkt[32];
    ...
    case CMD_BRIGHT:
        ...
        break;
    case CMD_ORG:
        txradio_send_packet(CMD_XYZ, CMD_OPCODE_X, 0);
        txradio_send_packet(CMD_ZOOM, CMD_OPCODE_ZOOM, 100);
        txradio_send_packet(CMD_BRIGHT, CMD_OPCODE_BRIGHT, 0);
        return;
    default:
        return;
    }
...
void txradio_init(void)
{
    ...
    xTaskCreate((void *)&cli_task, "CLI", 1024, NULL, 2, NULL);
}
