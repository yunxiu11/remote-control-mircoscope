...
QueueHandle_t s4831567QueueTrimpot;
...
void s4831567TaskTrimpot(void) {
    ...
    for (;;) {
        ...
        if (abs(adc_value - last_adc) > 10) {
            xQueueSend(s4831567QueueTrimpot, &adc_value, 0);
            last_adc = adc_value;
        }
...
void s4831567TaskTrimpotInit(void) {
    s4831567QueueTrimpot = xQueueCreate(4, sizeof(int));
    xTaskCreate((void*)&s4831567TaskTrimpot, "TrimpotTask", 256, NULL, 2, NULL);
}
